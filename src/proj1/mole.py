...
class Molecule(MoleculeMixin):
    ...
    def print_bond_length(self):
        ...
        for i in range(self.natm):
            for j in range(i + 1, self.natm):
                print("{:3d} - {:3d}: {:10.5f} Bohr".format(i,
                      j, self.bond_length(i, j)))
                ...
                for k in range(j + 1, self.natm):
                    for tup in [(i, j, k), (j, i, k), (i, k, j)]:
                        if self.is_valid_bond_angle(*tup):
                            print(
                                "{:3d} - {:3d} - {:3d}: {:10.5f} Degree".format(*tup, self.bond_angle(*tup)))
                            break
    ...
    def out_of_plane_angle(self, i: int, j: int, k: int, l: int) -> float:
        ...
        res = np.cross(self.bond_unit_vector(k, j), self.bond_unit_vector(
            k, l)).dot(self.bond_unit_vector(k, i))
                        ...
                        if self.is_valid_bond_angle(*tup):
                            for i in range(self.natm):
                                if i not in [j, k, l] and self.is_valid_out_of_plane_angle(i, *tup):
                                    print("{:3d} - {:3d} - {:3d} - {:3d}: {:10.5f} Degree".format(
                                        i, *tup, self.out_of_plane_angle(i, *tup)))

    def dihedral_angle(self, i: int, j: int, k: int, l: int) -> float:
        # Input: `i`, `j`, `k`, `l` index of molecule's atom; where `k` is the central atom, and angle is i - j-k-l
        # Output: Dihedral angle for atoms `i`-`j`-`k`-`l`
        res = np.cross(self.bond_unit_vector(j, i), self.bond_unit_vector(j, k)).dot(
            np.cross(self.bond_unit_vector(k, j), self.bond_unit_vector(k, l)))
        res /= np.sin(self.bond_angle(i, j, k) / 180 * np.pi) * \
            np.sin(self.bond_angle(j, k, l) / 180 * np.pi)
        assert(np.abs(res) < 1 + 1e-7)
        res = np.sign(res) if np.abs(res) > 1 else res
    ...
    def moment_of_inertia(self):
        ...
        res = - np.einsum("i, ix, iy -> xy", atom_weights,
    ...
    def print_solution_01(self):
        ...
        print("In cm^-1: {:12.5f} {:12.5f} {:12.5f}".format(*
              self.rotational_constants()))
        print("In   MHz: {:12.5f} {:12.5f} {:12.5f}".format(
            *(self.rotational_constants() * c_0 / centi / mega)))
